...
module.exports = (client) => {
  ...
  const discord_auth =
    discordAPI +
    `oauth2/authorize?client_id=${process.env.DISCORD_ID}&redirect_uri=${encodeURIComponent(siteURL)}%2Fauth&response_type=code&scope=identify%20guilds`;
  ...
  function canManageServer(guild) {
    ...
    return (
      guild.owner ||
      guild.permissions & manage_server ||
      guild.permissions & server_admin
    );
  }
  ...
  function clearDeletedData(settings, roles, channels) {
    if (roles) {
      settings.rewards = settings.rewards.filter((x) =>
        roles.some((r) => r.id == x.id),
      );
      settings.multipliers.roles = settings.multipliers.roles.filter((x) =>
        roles.some((r) => r.id == x.id),
      );
    }

    if (channels) {
      settings.multipliers.channels = settings.multipliers.channels.filter(
        (x) => channels.some((c) => c.id == x.id),
      );
      if (
        settings.levelUp.channel.length > 10 &&
        !channels.some((c) => c.id == settings.levelUp.channel)
      )
        settings.levelUp.channel = "current";
    }
  ...
  if (auth.supportURL)
    app.get("/support", (req, res) => res.redirect(auth.supportURL));
  if (auth.changelogURL)
    app.get("/changelog", (req, res) => res.redirect(auth.changelogURL));

  app.get("/invite/:id?", (req, res) => {
    res.redirect(
      `https://discord.com/oauth2/authorize?client_id=${client.user.id}&permissions=429765545024&scope=bot%20applications.commands${req.params.id ? `&guild_id=${req.params.id}` : ""}`,
    );
  });
  ...
  app.get("/api/loggedin", async function (req, res) {
    ...
    console.log(info);
    return res.send({
      login: info ? { id: info.id, username: info.username } : null,
      botPublic,
    });
  });
  ...
  app.get("/api/guilds", async function (req, res) {
    ...
    let foundServers = await client.db
      .find({ _id: { $in: guildList } }, "settings")
      .catch(() => []);
    let validServers = guilds.filter(
      (x) =>
        x.owner ||
        x.permissions & manage_server ||
        foundServers.some((g) => g._id == x.id),
    ); // filter to just the servers above, OR manageable servers

    let activeIDs = await client.shard.broadcastEval(
      async (cl, xd) => {
  ...
  app.get("/api/settings/:id", async function (req, res) {
        ...
        async (cl, xd) => {
          ...
          let channels = guild.channels.cache
            ...
            .sort((a, b) => {
              ...
              return (
                channelA.group - channelB.group ||
                channelA.section - channelB.section ||
    ...
    serverData.settings = clearDeletedData(
      serverData.settings,
      guildData.roles,
      guildData.channels,
    );
    ...
    if (serverData.settings.levelUp.embed && lvlMessage) {
      serverData.settings.levelUp.message = JSON.stringify(
        JSON.parse(lvlMessage),
        null,
        2,
      );
    }

    let ownedServers = guilds
      .filter((x) => x.owner && x.id != serverID)
    ...
    switch (setting.type) {
      ...
      case "float":
        let numVal =
          setting.type == "int"
            ? Math.round(val)
            : Number(Number(val).toFixed(setting.precision || 4));
        if (isNaN(numVal)) return setting.default;
        else return tools.clamp(numVal, setting.min, setting.max);
      case "string":
        let strVal = String(val)
          .slice(0, setting.maxlength || 64)
          .trim();
        if (setting.accept) {
          let acceptList = setting.accept.filter(
            (x) => !x.startsWith("discord:"),
          );
          if (setting.accept.includes("discord:role"))
            acceptList = acceptList.concat(guildData.roles);
          if (setting.accept.includes("discord:channel"))
            acceptList = acceptList.concat(guildData.channels);
      ...
      case "collection":
        ...
        val.forEach((x) => {
          ...
          Object.entries(x).forEach((s) => {
            ...
            if (foundSetting)
              validated[innerKey] = validateSetting(
                innerVal,
                foundSetting,
                guildData,
              );
  ...
  app.post("/api/settings", async function (req, res) {
    ...
    Object.entries(req.body).forEach((x) => {
      ...
      if (!setting) return;
        ...
        dbObj["settings." + setting.db] = validateSetting(
          val,
          setting,
          guildData,
        );
    ...
    if (dbObj["settings.levelUp.embed"] && dbObj["settings.levelUp.message"]) {
      ...
      if (lvlEmbed.invalid) {
        ...
        dbObj["settings.levelUp.message"] = JSON.stringify(lvlEmbed.json());
    }
    ...
    // prevent 0 curve
    if (
      dbObj["settings.curve.3"] == 0 &&
      dbObj["settings.curve.2"] == 0 &&
      dbObj["settings.curve.1"] == 0
    )
      dbObj["settings.curve.1"] = 1;

    dbObj["info.lastUpdate"] = Date.now();

    client.db
      .update(guildID, { $set: dbObj })
      .exec()
      .then(() => {
  ...
  app.post("/api/sendexample", async function (req, res) {
    ...
    let forceLevel = req.body.level
      ? tools.clamp(Math.round(req.body.level), 1, 1000)
      : null;
    ...
    let guildData = await client.shard
      .broadcastEval(
        async (cl, xd) => {
          ...
          let roles = guild.roles.cache.map((x) => ({
            name: x.name,
            id: x.id,
          }));
    ...
    let currentXP = forceLevel
      ? tools.xpForLevel(forceLevel, data.settings)
      : (data.users || [])[user.id]?.xp || 42069;
    let currentLevel =
      forceLevel || tools.getLevel(currentXP, data.settings) || 1;
    ...
    let msgData = {
      ...
      member: {
        displayName:
          guildData.member.displayName || user.global_name || user.username,
        avatarLink:
          guildData.member.avatar ||
      ...
      guild: {
        ...
        iconLink:
          guildData.server.icon ||
          `https://cdn.discordapp.com/icons/${foundGuild.id}/${foundGuild.icon || "nope"}`,
      },
      channel: {
    ...
    data.settings.levelUp.message = req.body.message.slice(
      0,
      schemaData.settings.levelUp.message.maxlength,
    );

    let lvlMessage = new LevelUpMessage(data.settings, msgData, {
      level: currentLevel,
      roleList: guildData.roles,
    ...
    fetchedUser
      ...
      .catch((e) => {
        fetchedUser
          .send(`**Error sending level up message!**\n\`\`\`${e.message}\`\`\``)
  ...
  app.post("/api/pruneMembers", async function (req, res) {
    ...
    if (req.body.confirmPrune != "hell yes") {
      ...
      client.db
        .update(guildID, { $set: { users: newUsers } })
        .exec()
        .then(() => {
          return res.end(
            `Successfully pruned ${toPrune} user${toPrune == 1 ? "" : "s"}!`,
          );
        })
        .catch(console.error);
    }
  ...
  app.post("/api/importfrombot", async function (req, res) {
    ...
    if (
      ...
      !req.body.import ||
      typeof req.body.import != "object"
    )
      return res.apiError("Invalid data!");
    ...
    if (!Object.keys(importSettings).some((x) => x != "bot"))
      return res.apiError("Invalid import options!");

    if (bot == "json") {
      if (!req.body.jsonData) return res.apiError("No .json data provided!");
      if (!isDev)
        importSettings.settings = null; // only devs can import settings
      else importSettings.isDev = true;
    }

    let importCode = `${guildID}-${bot}`;
    if (
      importCooldowns[importCode] &&
      importCooldowns[importCode] >= Date.now()
    )
      return res.apiError(
        `Please wait ${tools.time(importCooldowns[importCode] - Date.now(), 1)} seconds before the next import!`,
        "importCooldown",
      );

    let newData;
    if (bot == "polaris")
      newData = await require("./commands/misc/polaris_transfer.js").run(
        client,
        guildID,
        importSettings,
        guilds,
      ...
      newData = await require("./commands/misc/json_import.js").run(
        client,
        guildID,
        importSettings,
        req.body.jsonData,
    ...
    if (newData && newData.error)
      return res.apiError(newData.error, newData.code);
    else if (!newData || !newData.data || !Object.keys(newData.data).length)
      return res.apiError("No data!", "noData");
    else {
      client.db
        .update(guildID, { $set: newData.data })
        .exec()
        .then(() => {
        ...
        .catch((e) => {
          ...
          if (bot == "json")
            return res.apiError(
              `The database rejected your json! Are the settings you provided valid? (Error: ${e.message})`,
            );
  ...
  app.get("/api/leaderboard/:id", cors(), async function (req, res) {
    ...
    let singleUser =
      req.query.user && req.query.user.match(/^\d{10,30}$/)
        ? req.query.user
        : null;

    let [userInfo, guildInfo] = singleUser
      ? [null, null]
      : await getDiscordInfo(req);
    ...
    let minLeaderboardXP =
      settings.leaderboard.minLevel > 1
        ? tools.xpForLevel(settings.leaderboard.minLevel, settings)
        : 0;

    let rewardList = settings.leaderboard.hideRoles
      ? []
      : settings.rewards.filter((x) => x.level <= settings.maxLevel);
    let importantRoles = settings.multipliers.roles
      .concat(rewardList)
      .map((x) => x.id);
    ...
    if (hiddenMembers.length)
      hiddenMembers = hiddenMembers.sort((a, b) => b.xp - a.xp);
    ...
    if (minLeaderboardXP > 0)
      xpArray = xpArray.filter((x) => x.xp > minLeaderboardXP);
    if (settings.leaderboard.maxEntries > 0)
      xpArray = xpArray.slice(0, settings.leaderboard.maxEntries);

    let pageSize = tools.clamp(parseInt(req.query.pageSize || 100), 1, 100);
    let pageCount = Math.ceil(xpArray.length / pageSize);
    let page = Math.max(1, parseInt(req.query.page)) || 1;

    let paginated = xpArray.slice(
      (page - 1) * pageSize,
      ...
      if (foundInPage) userLevel = foundInPage;
        ...
        if (!foundAsMember && isInGuild)
          foundAsMember = {
            ...
            color: "#" + (userInfo.accent_color || 0xffffff).toString(16),
            avatar: `https://cdn.discordapp.com/avatars/${userInfo.id}/${userInfo.avatar}`,
            roles: [],
          };
        if (foundUser && foundAsMember)
          userLevel = Object.assign(foundUser, foundAsMember);
        else
          userLevel = {
            missing: true,
            debug: {
              member: guildMembers.find((x) => x.id == userInfo.id)?.id || "-",
              user: foundUser?.id || "-",
              info: userInfo?.id || "-",
            },
          };
    ...
    let importantSettings = {
      ...
      multipliers: {
        roles: settings.multipliers.roles,
        rolePriority: settings.multipliers.rolePriority,
      },
  ...
  app.post("/api/editXP", async function (req, res) {
    ...
    if (!foundXP?.users || !foundXP.users[req.body.user])
      return res.apiError("This user isn't ranked!");

    client.db
      .update(guildID, { $set: { [`users.${req.body.user}.xp`]: newXP } })
      .exec()
      .then(() => res.end("cool and good"))
  ...
  app.post("/api/leaderboardHide", async function (req, res) {
    ...
    client.db
      .update(guildID, { $set: { [`users.${req.body.user}.hidden`]: setHide } })
      .exec()
      .then(() => res.end("cool and good"))
      .catch(() => res.apiError("Unknown error!"));
  });
  ...
  app.get("/api/xp/:id", cors(), async function (req, res) {
    ...
    switch (format) {
      ...
      case "csv": // spreadsheet
        ...
        return res.send(
          header + xpList.map((x) => `${x.id},${x.xp}`).join("\n"),
        );
...
By authorizing, they give permission for us to view basic info and see what servers they're in (identify + guilds) */
  app.get("/discord", function (req, res) {
    return res.redirect(discord_auth);
  });
  ...
  app.get("/auth", async function (req, res) {
    ...
    if (req.query.code && req.query.code.length > 10) {
      ...
      fetch(discordAPI + "/oauth2/token", {
        ...
        .then((data) => {
          if (data["error"])
            return sendRedirect(res, "/"); // if discord sends an error (fake code, etc)
    ...
    return await authDB
      ...
      .then((data) => {
        ...
        if (data && data.access_token && data.expires > Date.now()) {
          // if a token was found, cache and return it
...
Now we can make requests to Discord to get the client's stuff! */
  ...
  async function getDiscordInfo(req, userOnly) {
    ...
    let guilds = await rest
      .get("/users/@me/guilds", options)
      .catch((e) => null);
    ...
    if (
      ...
      return [null, null]; // if discord sends error
...
If the client wants to log out, we should probably respect that and delete their stuff */
  app.get("/logout", async function (req, res) {
    ...
    fetch(discordAPI + "oauth2/token/revoke", {
      ...
      .then(() => {
        // discord has invalidated the token!
  ...
  app.listen(auth.serverPort, () =>
    console.log(
      `Web server online at http://localhost:${auth.serverPort} (${+process.uptime().toFixed(2)} secs)`,
    ),
  );
};
