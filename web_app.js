...
module.exports = (client) => {
  ...
  const discord_auth =
    ...
    `oauth2/authorize?client_id=${
      ...
      siteURL
    )}%2Fauth&response_type=code&scope=identify%20guilds`;
  ...
  app.get("/invite/:id?", (req, res) => {
    res.redirect(
      `https://discord.com/oauth2/authorize?client_id=${
        client.user.id
      }&permissions=429765545024&scope=bot%20applications.commands${
    ...
    let msgData = {
      ...
      guild: {
        ...
        iconLink:
          ...
          `https://cdn.discordapp.com/icons/${foundGuild.id}/${
            foundGuild.icon || "nope"
          }`,
      },
      channel: {
  ...
  app.post("/api/importfrombot", async function (req, res) {
    ...
    if (
      ...
      return res.apiError(
        `Please wait ${tools.time(
          importCooldowns[importCode] - Date.now(),
          1
        )} seconds before the next import!`,
        "importCooldown"
      );
...
If the client wants to log out, we should probably respect that and delete their stuff */
  ...
  app.listen(auth.serverPort, () =>
    console.log(
      `Web server online at http://localhost:${auth.serverPort} (${+process
        .uptime()
        .toFixed(2)} secs)`
    )
  );
