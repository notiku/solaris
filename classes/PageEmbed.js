...
class PageEmbed {
  ...
  paginate(pg = this.page) {
    return this.fullData.slice(
      (pg - 1) * this.size,
      (pg - 1) * this.size + this.size,
    );
  }
  ...
  setDesc() {
    ...
    if (typeof this.mapFunction == "function")
      currentData = currentData.map((x, y) => {
  ...
  post(int, msgSettings = {}) {
    ...
    let pageOptions = [
      {
        style: firstPage ? "Secondary" : "Success",
        label: `<< Page ${firstPage ? this.pages : Math.max(this.page - 1 || 1, 1)}`,
        customId: "prev",
      },
      {
        style: lastPage ? "Secondary" : "Success",
        label: `Page ${lastPage ? 1 : Math.min(this.page + 1, this.pages)} >>`,
        customId: "next",
      },
    ];
    ...
    let pageButtons =
      this.pages <= 1
        ? this.extraButtons
        : tools.button(pageOptions).concat(this.extraButtons);
    ...
    if (!this.int)
      ...
      return this.int
        .editReply({ embeds: [this.embed], components: pgButtonRow })
        .then((msg) => {
  ...
  handleButtons(msg, buttons) {
    ...
    let collector = msg.createMessageComponentCollector({
      time: this.timeoutSecs * 1000,
    });
    if (this.ownerID) activeCollectors[this.ownerID] = collector;
    collector.on("collect", (b) => {
      if (buttonPressed || !tools.canPressButton(b, [this.ownerID]))
        return tools.buttonReply(b);
