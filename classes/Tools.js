...
class Tools {
  constructor(client, int) {
    ...
    if (!this.WEBSITE.startsWith("http"))
      this.WEBSITE = "https://gdcolon.com/polaris"; // backup URL or some buttons will break
    ...
    this.errors = {
      xpDisabled: `XP is not enabled in this server!${
        this.canManageServer()
          ? ` (enable with ${this.commandTag("config")})`
          : ""
      }`,
    ...
    this.fetchSettings = async function (userID, serverID = int.guild.id) {
      let data = await client.db.fetch(serverID, [
        "settings",
    ...
    this.getLevel = function (xp, settings, returnRequirement) {
      ...
      while (xp >= xpRequired && lvl <= settings.maxLevel) {
        // cubic formula my ass, here's a while loop. could probably binary search this?
        lvl++;
      ...
      return returnRequirement
        ? { level: lvl, xpRequired, previousLevel }
        : lvl;
    };
    ...
    this.getRolesForLevel = function (lvl, rewards) {
      ...
      let levelRoles = rewards
        .filter((x) => x.level <= lvl) // get all reward roles less than or equal to level
    ...
    this.getMultiplier = function (member, settings, channel = int.channel) {
      ...
      let foundChannelBoost = channelIDs
        .map((x) => settings.multipliers.channels.find((c) => c.id == x))
        .find((x) => x);
      ...
      if (roleBoosts.length) {
        ...
        if (foundXPBan) foundRoleBoost = foundXPBan;
          ...
          switch (obj.rolePriority) {
            case "smallest": // lowest boost
              foundRoleBoost = roleBoosts.sort((a, b) => a.boost - b.boost)[0];
              break;
            case "highest": // highest role
              let foundTopBoost = memberRoles
                .sort((a, b) => b.position - a.position)
                .find((x) => roleBoosts.find((y) => y.id == x.id));
              foundRoleBoost = roleBoosts.find((x) => x.id == foundTopBoost.id);
              break;
            case "combine": // multiply all, holy shit
              let combined = roleBoosts
                .map((x) => x.boost)
                .reduce((a, b) => a * b, 1)
                .toFixed(4);
              combined = Math.min(+combined, 1000000); // 1 million max
              obj.role = combined;
              obj.roleList = roleBoosts;
              break;
            case "add": // add (n-1) from each
              let filteredBoosts = roleBoosts.filter((x) => x.boost != 1);
              let summed =
                filteredBoosts.length == 1
                  ? filteredBoosts[0].boost
                  : filteredBoosts
                      .map((x) => x.boost)
                      .reduce((a, b) => a + (b - 1), 1);
              obj.role = Number(summed.toFixed(4));
              obj.roleList = filteredBoosts;
              break;
            default: // largest boost
              obj.rolePriority = "largest";
              foundRoleBoost = roleBoosts.sort((a, b) => b.boost - a.boost)[0];
              break;
          }
      ...
      if (obj.role <= 0 || obj.channel <= 0)
        obj.multiplier = 0; // 0 always takes priority
      else
        switch (settings.multipliers.channelStacking) {
          case "largest":
            obj.multiplier = Math.max(obj.role, obj.channel);
            break; // pick largest between channel and role
          case "channel":
            obj.multiplier = foundChannelBoost ? obj.channel : obj.role;
            break; // channel always takes priority if it exists
          case "role":
            obj.multiplier = foundRoleBoost ? obj.role : obj.channel;
            break; // role takes priority if it exists
          case "add":
          ...
          default:
            obj.channelStacking = "multiply";
            obj.multiplier = obj.role * obj.channel;
            break; // just multiply them together
        }
    ...
    this.confirmButtons = function (
      ...
      return this.button([
        {
          style: titleColor || "Success",
          label: titleText || "Confirm",
          customId: "confirm",
        },
        {
          style: cancelColor || "Danger",
    ...
    this.row = function (components) {
      if (!components || (Array.isArray(components) && !components[0]))
        return null;
          ...
          if (activeConfirmation) {
            msg
              .edit({
                ...
                components: this.disableButtons(confirmBtns),
              })
              .catch(() => {});
    ...
    this.time = function (ms, decimals = 0, noS, shortTime) {
      ...
      if (seconds < 1)
        return timeFormat((ms / 1000).toFixed(2), shortTime ? "sec" : "second");
      if (seconds < 60)
        return timeFormat(seconds, shortTime ? "sec" : "second");
      else if (minutes < 60)
        return timeFormat(minutes, shortTime ? "min" : "minute");
    ...
    this.timestamp = function (ms, useTimeIfLong) {
      ...
      let timestamp = `${ms < 0 ? "-" : ""}${days ? `${days}d + ` : ""}${[
    ...
    this.jsonImport = function (serverID, url, xpKey = "xp", idKey = "id") {
      fetch(url)
        .then((res) => res.json())
        .then((list) => {
          let xpStuff = list.map((x) => ({
            xp: Math.round(x[xpKey]),
            id: x[idKey],
          }));
          let users = {};
          xpStuff.forEach((x) => (users[x.id] = { xp: x.xp }));
          client.db
            .update(serverID, { $set: { users } })
            .exec()
            .then(() => {
              int.channel.send({ content: "Success!" });
            });
        })
        .catch((e) => {
          int.channel.send({ content: "Failed! " + e.message });
        });
    };
  }
}
