...
class LevelUpMessage {
  constructor(settings, message, data = {}) {
    ...
    this.rewardRoles = settings.rewards
      .filter((x) => x.level == data.level)
      .map((x) => roleList.find((r) => r.id == x.id))
      .filter((x) => x);

    if (
      settings.levelUp.rewardRolesOnly &&
      !this.rewardRoles.length &&
      !data.example
    ) {
      this.invalid = true;
      return;
    ...
    this.variables = {
      ...
      NEXT_XP: tools.commafy(
        tools.xpForLevel(data.level + 1, settings) - data.userData.xp,
      ),
      "@": `<@${message.author.id}>`,
      ...
      AVATAR:
        message.member.avatarLink ||
        message.member.displayAvatarURL({ format: "png", dynamic: true }),
      SERVER: message.guild.name,
      SERVER_ID: message.guild.id,
      SERVER_ICON:
        message.guild.iconLink ||
    ...
    if (settings.levelUp.embed) {
      ...
      if (mbed.invalid) {
          ...
          // go one extra layer deep lmao
            ...
            for (const [key2, val2] of Object.entries(val)) {
              if (typeof val2 == "string")
                mbedJSON[key][key2] = this.subVariables(val2);
            }
          }
        }
        ...
        if (mbedJSON.fields && mbedJSON.fields.length) {
          mbedJSON.fields = mbedJSON.fields.map((f) => ({
            name: this.subVariables(f.name),
            value: this.subVariables(f.value),
            inline: f.inline,
          }));
        }

        this.msg = { embeds: [mbedJSON] };
        if (mbed.extraContent)
          this.msg.content = this.subVariables(mbed.extraContent);
      }
  ...
  subVariables(msg) {
    ...
    newMsg = newMsg.replace(/\[\[\s*CHOOSE.+?\]\]/g, function (str) {
      ...
      let choose = str
        ...
        .split(/(?<!\|)\|(?!\|)/)
        .map((x) => x.trim())
        .filter((x) => x); // split at one | but not more
      choose[0] = choose[0].replace(/^\s*CHOOSE\s*/, "");
      if (!choose[0]) choose.shift();

      let chooseRegex = /^<([\d.]+)>\s+/;
      if (choose.some((x) => x.match(chooseRegex))) {
        // if list has weighting...
        choose.forEach((c) => {
    ...
    // nth
    newMsg = newMsg
      .replace(new RegExp(ordinalRegex, "g"), function (str) {
  ...
  async send() {
    ...
    let ch =
      sendChannel == "current"
        ? this.userMessage.channel
        : sendChannel == "dm"
          ? this.userMessage.author
          : await this.userMessage.guild.channels
              .fetch(sendChannel)
              .catch(() => {});
