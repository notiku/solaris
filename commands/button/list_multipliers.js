...
module.exports = {
  ...
  async run(client, int, tools) {
    ...
    if (!mList.length)
      return tools.warn(`This server doesn't have any ${mType} multipliers!`);
    ...
    let categories;
    if (isChannel) {
      categories = await int.guild.channels
        .fetch()
        .then((x) =>
          x
    ...
    let multiplierEmbed = new PageEmbed(embed, multipliers, {
      ...
      owner: int.user.id,
      mapFunction: (x) =>
        `**${x.boost}x:** ${isChannel ? (categories.includes(x.id) ? `**<#${x.id}>** (category)` : `<#${x.id}>`) : `<@&${x.id}>`}`,
    });
