...
module.exports = {
  ...
  async run(client, int, tools, modal) {
    ...
    if (!modal) {
      if (isNumber) {
        ...
        if (!isNaN(setting.min) && !isNaN(setting.max))
          numOption.setPlaceholder(
            `${tools.commafy(setting.min)} - ${tools.commafy(setting.max)}`,
          );
    ...
    client.db
      ...
      .then(() => {
        client.commands
          .get("button:settings_view")
          .run(client, int, tools, ["val", null, settingID]);
      })
      .catch(() =>
        tools.warn("Something went wrong while trying to change this setting!"),
      );
  },
};
