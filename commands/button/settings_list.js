...
module.exports = {
  ...
  async run(client, int, tools, selected) {
    ...
    if (int.isButton) {
      ...
      if (buttonData[2] && buttonData[2] != int.user.id)
    ...
    function previewSetting(val, data, schema) {
      ...
      switch (schema.type) {
        case "bool":
          return (data.invert ? !val : val) ? "__True__" : "False";
        case "int":
          return tools.commafy(val);
        case "float":
          return tools.commafy(Number(val.toFixed(schema.precision || 4)));
      }
      return val.toString();
    }
    ...
    let dirName =
      (selected ? selected[1] : int.isButton ? buttonData[1] : rootFolder) ||
      rootFolder;
    ...
    entries.forEach((x) => {
      ...
      if (x.folder) {
        ...
        options.push({
          emoji: getDataEmoji(sch.type, val),
          label: x.name,
          description: tools.limitLength(x.desc, 95),
          value: `config_val_${dirName}_${x.db}`,
        });
      }
    ...
    tools.editOrReply(
      { embeds: [embed], components: tools.row(dropdown) },
      !buttonData[2],
    );
  },
};
