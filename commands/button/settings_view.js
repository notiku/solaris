  ...
  async run(client, int, tools, selected) {
    ...
    function previewSetting(val) {
      if (data.zeroText && val === 0) return `0 (${data.zeroText})`;
        ...
        switch (setting.type) {
          case "bool":
            return (data.invert ? !val : val) ? "True" : "False";
          case "int":
            return tools.commafy(+val);
          case "float":
            return tools.commafy(Number(val.toFixed(setting.precision || 8)));
        }
      return val.toString();
    }
    ...
    let buttons = tools.button([
      {
        style: "Secondary",
        label: "Back",
        customID: `settings_list~${group}~${int.user.id}`,
      },
      {
        style: "Primary",
        label: setting.type == "bool" ? "Toggle" : "Edit",
        customId: `settings_edit~${settingID}~${int.user.id}`,
      },
    ]);
