...
module.exports = {
    ...
    args: [
      {
        type: "integer",
        name: "page",
        description: "Which page to view (negative to start from last page)",
        required: false,
      },
      {
        type: "user",
        name: "member",
        description:
          "Finds a certain member's position on the leaderboard (overrides page)",
        required: false,
      },
      {
        type: "bool",
  ...
  async run(client, int, tools) {
    ...
    if (!db || !db.users || !Object.keys(db.users).length)
      return tools.warn(`Nobody in this server is ranked yet!`);
    else if (!db.settings.enabled) return tools.warn("*xpDisabled");
    else if (db.settings.leaderboard.disabled)
      return tools.warn(
        "The leaderboard is disabled in this server!" +
          (tools.canManageServer(int.member)
    ...
    let minLeaderboardXP =
      db.settings.leaderboard.minLevel > 1
    ...
    if (userSearch) {
      ...
      if (isNaN(foundRanking) || foundRanking < 0)
        return tools.warn(
          int.user.id == userSearch.user.id
            ? "You aren't on the leaderboard!"
            : "This member isn't on the leaderboard!",
        );
    ...
    let embed = tools.createEmbed({
      ...
      author: {
        name: "Leaderboard for " + int.guild.name,
        iconURL: int.guild.iconURL(),
      },
    });

    let isHidden =
      db.settings.leaderboard.ephemeral || !!int.options.get("hidden")?.value;
    ...
    let xpEmbed = new PageEmbed(embed, rankings, {
      ...
      ephemeral: isHidden,
      mapFunction: (x, y, p) =>
        `${p}) <@${x.id}> **level ${tools.getLevel(x.xp, db.settings)}** (\`${tools.commafy(x.xp)} XP\`)${x.id == highlight ? "**" : ""}`,
      extraButtons: [
        tools.button({
          style: "Link",
    ...
    if (!xpEmbed.data.length)
      return tools.warn("There are no members on this page!");
