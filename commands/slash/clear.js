module.exports = {
  metadata: {
    ...
    name: "clear",
    description:
      "Clear a member's cooldown. (requires manage server permission)",
    args: [
      {
        type: "user",
        name: "member",
        description: "Which member to clear",
        required: true,
      },
    ],
  },
  ...
  async run(client, int, tools) {
    ...
    client.db
      .update(int.guild.id, { $set: { [`users.${user.id}.cooldown`]: 0 } })
      .then(() => {
        int.reply(
          `🔄 **${tools.pluralS(user.displayName)} cooldown has been reset!** (previously ${tools.timestamp(cooldown - Date.now())})`,
        );
      })
      .catch(() =>
