module.exports = {
  metadata: {
    ...
    name: "addxp",
    description:
      "Add or remove XP from a member. (requires manage server permission)",
    args: [
      {
        type: "user",
        name: "member",
        description: "Which member to modify",
        required: true,
      },
      {
        type: "integer",
        name: "xp",
        description: "How much XP to add (negative number to remove XP)",
        min: -1e10,
        max: 1e10,
        required: true,
      },
      {
        type: "string",
        name: "operation",
        description: "How the XP amount should be interpreted",
        required: false,
        choices: [
  ...
  async run(client, int, tools) {
    ...
    if (amount === 0 && operation.startsWith("add"))
      return tools.warn("Invalid amount of XP!");
    ...
    client.db
      .update(int.guild.id, { $set: { [`users.${user.id}.xp`]: newXP } })
      .then(() => {
        int.reply(
          `${newXP > xp ? "⏫" : "⏬"} ${user.displayName} now has **${tools.commafy(newXP)}** XP${newLevel != level ? ` and is **level ${newLevel}**` : ""}! (previously ${tools.commafy(xp)}, ${xpDiff >= 0 ? "+" : ""}${tools.commafy(xpDiff)})`,
        );
      })
      .catch(() =>
        tools.warn("Something went wrong while trying to modify XP!"),
      );
  },
};
