  ...
  switch (arg.type) {
    ...
    case "channel":
      return data.addChannelOption((option) => {
        ...
        else if (arg.acceptAll)
          option.addChannelTypes([0, 2, 4, 5, 10, 11, 12, 13, 15, 16]); // lol
        else
...
module.exports = {
    ...
    args: [
      {
        type: "bool",
        name: "global",
        description: "Publish the public global commands instead of dev ones",
        required: false,
      },
      {
        type: "string",
        name: "server_id",
        description: "Deploy dev commands to a specific server",
        required: false,
      },
      {
        type: "bool",
        name: "undeploy",
        description:
          "Clears all dev commands from the server (or global if it's set to true)",
        required: false,
      },
    ],
  },
  ...
  async run(client, int, tools) {
    ...
    let targetServer =
      !int || isPublic ? null : int.options.get("server_id")?.value;
          ...
          case "slash": // slash commands
            ...
            if (metadata.dev) data.setDefaultMemberPermissions(0);
              ...
              data.setDefaultMemberPermissions(
                Discord.PermissionFlagsBits[metadata.permission],
              );
    ...
    if (isPublic) {
      ...
      rest
        .put(route, { body: interactionList })
        .then(() => {
          if (int)
            int.reply(
              `**${!undeploy ? `${interactionList.length} global commands registered!` : "Global commands cleared!"}** (Wait a bit, or refresh with Ctrl+R to see changes)`,
            );
          else console.info("Global commands registered!");
          client.shard.broadcastEval((cl) => {
            cl.application.commands.fetch();
            return;
          }); // cache new slash commands
        })
        .catch((e) =>
          console.error(
            `Error deploying global commands to ${id}: ${e.message}`,
          ),
        );
    } else {
      let serverIDs = targetServer
        ? [targetServer]
        : int?.guild
          ? [int.guild.id]
          : config.test_server_ids;
      if (!serverIDs)
        return console.warn(
          "Cannot deploy dev commands! No test server IDs provided in config.",
        );
        ...
        rest
          ...
          .then(() => {
            ...
            if (int)
              int.reply(
                undeploy
                  ? "Dev commands cleared!"
                  : id == int.guild.id
                    ? "Dev commands registered!"
                    : msg,
              );
          ...
          .catch((e) =>
            console.error(
              `Error deploying dev commands to ${id}: ${e.message}`,
            ),
          );
      });
    }
