...
module.exports = {
  metadata: {
    ...
    args: [
      ...
      },
      {
        type: "user",
        name: "member",
        description: "Which member to check",
        required: false,
      },
    ],
  },
  ...
  async run(client, int, tools) {
    ...
    let targetLevel = Math.min(
      int.options.get("target").value,
      db.settings.maxLevel
    );
    ...
    if (targetLevel == userLevel && userLevel >= db.settings.maxLevel)
      progressBar += `\n🎉 You reached the maximum level${
        db.settings.maxLevel < 1000 ? " in this server" : ""
      }! Congratulations!`;

    let multiplierData = tools.getMultiplier(member, db.settings);
    let multiplier = multiplierData.multiplier || multiplierData.role;
    if (multiplier <= 0)
      return int.reply("Your multiplier prevents you from gaining any XP!");

    let estimatedMin = Math.ceil(
      remaining / (db.settings.gain.min * multiplier)
    );
    let estimatedMax = Math.ceil(
      remaining / (db.settings.gain.max * multiplier)
    );
    ...
    let estimatedRange =
      estimatedMax == estimatedMin
        ...
        : `${tools.commafy(estimatedMax)} - ${tools.commafy(
            estimatedMin
          )} (avg. ${tools.commafy(estimatedAvg)})`;

    let levelDetails = [
      `**Current XP: **${tools.commafy(xp)} (Level ${tools.commafy(
        userLevel
      )})`,
      `**Target XP: **${tools.commafy(targetXP)}`,
      `**Remaining XP: **${reached ? "0 (" : ""}${tools.commafy(
        targetXP - xp
      )}${reached ? ")" : ""}`,
    ];
    ...
    if (!reached)
      levelDetails = levelDetails.concat([
        "",
        `**XP per message: **${
          db.settings.gain.min == db.settings.gain.max
            ? tools.commafy(Math.round(db.settings.gain.min * multiplier))
            : `${tools.commafy(
                Math.round(db.settings.gain.min * multiplier)
              )} - ${tools.commafy(
                Math.round(db.settings.gain.max * multiplier)
              )}`
        }`,
        ...
        `**Cooldown remaining: **${
          estimatedTime == Infinity
            ...
            : tools.time(estimatedTime * 1000, 1)
        }`,
      ]);
