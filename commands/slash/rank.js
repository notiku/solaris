...
module.exports = {
  ...
  async run(client, int, tools) {
    ...
    let progressBar = `${"▓".repeat(barRepeat)}${"░".repeat(
      barSize - barRepeat
    )} (${!maxLevel ? Number(levelPercent.toFixed(2)) + "%" : "MAX"})`;
    ...
    let estimatedRange =
      estimatedMax == estimatedMin
        ? `${tools.commafy(estimatedMax)} ${tools.extraS(
            "message",
            estimatedMax
          )}`
        : `${tools.commafy(estimatedMax)}-${tools.commafy(
            estimatedMin
          )} messages`;
    ...
    let nextLevelXP =
      (db.settings.rankCard.relativeLevel
        ? `${tools.commafy(xp - levelData.previousLevel)}/${tools.commafy(
            levelData.xpRequired - levelData.previousLevel
          )}`
    ...
    if (
      ...
      if (multRoles.length)
        multiplierInfo.push(
          `**Total multiplier: ${multiplier}x XP** (${multiplierModes.channelStacking[
            multiplierData.channelStacking
          ].toLowerCase()})`
        );
    }
    ...
    if (multiplierInfo.length)
      ...
      if (syncCheck.incorrect.length || syncCheck.missing.length)
        embed.addFields([
          {
            name: "⚠ Note",
            value: `Your level roles are not properly synced! Type ${tools.commandTag(
              "sync"
            )} to fix this.`,
          },
        ]);
