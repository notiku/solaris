...
module.exports = {
    ...
    args: [
      {
        type: "user",
        name: "member",
        description: "Which member to view",
        required: false,
      },
      {
        type: "bool",
        name: "hidden",
        description: "Hides the reply so only you can see it",
        required: false,
      },
    ],
  },
  ...
  async run(client, int, tools) {
    ...
    if (db.settings.rankCard.disabled)
      return tools.warn("Rank cards are disabled in this server!");

    // if user has no xp, stop here
    if (!currentXP || !currentXP.xp)
      return tools.noXPYet(foundUser ? foundUser.user : int.user);
    ...
    let levelPercent = maxLevel
      ? 100
      : ((xp - levelData.previousLevel) /
          (levelData.xpRequired - levelData.previousLevel)) *
        100;
    ...
    let estimatedMin = Math.ceil(
      remaining / (db.settings.gain.min * (multiplier || multiplierData.role)),
    );
    let estimatedMax = Math.ceil(
      remaining / (db.settings.gain.max * (multiplier || multiplierData.role)),
    );

    // estimated number of messages to level up
    let estimatedRange =
      estimatedMax == estimatedMin
        ? `${tools.commafy(estimatedMax)} ${tools.extraS("message", estimatedMax)}`
        : `${tools.commafy(estimatedMax)}-${tools.commafy(estimatedMin)} messages`;

    // xp required to level up
    let nextLevelXP =
      (db.settings.rankCard.relativeLevel
        ? `${tools.commafy(xp - levelData.previousLevel)}/${tools.commafy(levelData.xpRequired - levelData.previousLevel)}`
        : `${tools.commafy(levelData.xpRequired)}`) +
      ` (${tools.commafy(remaining)} more)`;
    ...
    let memberColor =
      cardCol ||
      member.displayColor ||
      (await member.user.fetch().then((x) => x.accentColor));
    ...
    let embed = tools.createEmbed({
      ...
      footer: maxLevel
        ? progressBar
        : estimatedMin == Infinity || estimatedMin < 0
          ? "You are unable to gain XP!"
          : `${progressBar}\n${estimatedRange} to go!`,
      fields: [
        {
          name: "✨ XP",
          value: `${tools.commafy(xp)} (lv. ${levelData.level})`,
          inline: true,
        },
        {
          name: "⏩ Next level",
          value: !maxLevel ? nextLevelXP : "Max level! Woah!",
          inline: true,
        },
      ],
    });
    ...
    if (!db.settings.rankCard.hideCooldown) {
      ...
      let cooldown =
        foundCooldown > Date.now()
          ? tools.timestamp(foundCooldown - Date.now())
          : "None!";
    ...
    if ((!hideMult || multiplierData.role == 0) && multRoles.length) {
      let xpStr =
        multiplierData.role > 0
          ...
          : "Cannot gain XP!";
      let roleMultiplierStr =
        multRoles.length == 1
          ? `${int.guild.id != multRoles[0].id ? `<@&${multRoles[0].id}>` : "Everyone"} - ${xpStr}`
          : `**${multRoles.length} roles** - ${xpStr}`;
    ...
    if (
      ...
      if (multRoles.length)
        multiplierInfo.push(
          `**Total multiplier: ${multiplier}x XP** (${multiplierModes.channelStacking[multiplierData.channelStacking].toLowerCase()})`,
        );
    }

    if (multiplierInfo.length)
      embed.addFields([
        { name: "🌟 Multiplier", value: multiplierInfo.join("\n") },
      ]);
    else if (
      !db.settings.rewardSyncing.noManual &&
      !db.settings.rewardSyncing.noWarning
    ) {
      let syncCheck = tools.checkLevelRoles(
        int.guild.roles.cache,
        member.roles.cache,
        levelData.level,
        db.settings.rewards,
      );
      if (syncCheck.incorrect.length || syncCheck.missing.length)
        embed.addFields([
          {
            name: "⚠ Note",
            value: `Your level roles are not properly synced! Type ${tools.commandTag("sync")} to fix this.`,
          },
        ]);
    }

    let isHidden =
      db.settings.rankCard.ephemeral || !!int.options.get("hidden")?.value;
