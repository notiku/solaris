...
module.exports = {
    ...
    args: [
      {
        type: "string",
        name: "property",
        description: "Property name (e.g. settings.enabled)",
        required: false,
      },
      {
        type: "string",
        name: "new_value",
        description: "New value for the property (parsed as JSON)",
        required: false,
      },
      {
        type: "string",
        name: "guild_id",
        description: "Guild ID to use (defaults to current guild)",
        required: false,
      },
    ],
  },
  ...
  async run(client, int, tools) {
    ...
    if (!propertyName) {
      ...
      Promise.resolve()
        .then(() => eval(`db.${propertyName.value}`)) // lmao
        .then((x) => int.reply(tools.limitLength(util.inspect(x), 1900)))
        .catch((e) => int.reply(`**Error:** ${e.message}`));
    } else {
      let val = newValue.value;
      try {
        val = JSON.parse(newValue.value);
      } catch (e) {
        newValue.value;
      }

      let confirmMsg = {
        content: `Click to update **${propertyName.value}** to: [${typeof val}] ${tools.limitLength(JSON.stringify(val), 256)}`,
      };
      tools.createConfirmationButtons({
        message: confirmMsg,
        buttons: "Update!",
        secs: 30,
        timeoutMessage: "Update cancelled",
        onClick: function (confirmed, msg, b) {
          if (!confirmed) return msg.reply("Update cancelled");
          else {
            client.db
              .update(guildID, { $set: { [propertyName.value]: val } })
              .exec()
              .then(() => {
