...
module.exports = {
  ...
  async run(client, int, tools) {
    let versionNumber =
      client.version.version != Math.round(client.version.version)
        ? client.version.version
        : client.version.version.toFixed(1);

    let stats = await client.shard.broadcastEval((cl) => ({
      guilds: cl.guilds.cache.size,
      users: cl.users.cache.size,
    }));
    ...
    let botStatus = [
      ...
      `**Servers:** ${tools.commafy(totalServers)}${
        client.shard.count == 1
          ? ""
          : ` (on shard: ${tools.commafy(client.guilds.cache.size)})`
      }`,
      `**Memory usage:** ${Number(
    ...
    let infoButtons = [
      { style: "Link", label: "Website", url: `${tools.WEBSITE}` },
    ];
    ...
    int
      .reply({
        embeds: [embed],
        components: tools.row(tools.button(infoButtons)),
        fetchReply: true,
      })
      .then((msg) => {
        embed.setFooter({
          text: `Ping: ${tools.commafy(
            msg.createdTimestamp - int.createdAt
          )}ms`,
        });
