...
module.exports = {
  async run(client, serverID, importSettings = {}, guilds) {
    ...
    else if (foundServer && !foundServer.owner)
      return { error: "Not owner in server" };
    else if (foundServer.id == serverID)
    ...
    if (!transferData)
      return {
        error: `No Polaris data found for ${foundServer.name}`,
        code: "invalidImport",
      };
    ...
    if (importSettings.xp) {
      ...
      Object.entries(transferData.users).forEach((u) => {
        importedUsers++;
        let xpVal = { xp: u[1].xp };
        if (u[1].cooldown && u[1].cooldown > now)
          xpVal.cooldown = u[1].cooldown;
        newData[`users.${u[0]}`] = xpVal;
      });
      details.push(
        `${tools.commafy(importedUsers)} user${importedUsers == 1 ? "" : "s"}`,
      );
    }

    if (importSettings.settings) {
      let currentSettings = await client.db
        .fetch(serverID, "settings")
        .then((x) => x.settings);
      ...
      transferSettings.multipliers.channels =
        currentSettings.multipliers.channels;
      if (transferSettings.levelUp.channel.length > 8)
        transferSettings.levelUp.channel = currentSettings.levelUp.channel;
