...
module.exports = {
  async run(client, serverID, importSettings = {}, jsonData) {
    ...
    if (
      ...
      jsonData = { users: jsonData }; // if no keys provided, assume it's just XP

    if (jsonData.users && importSettings.xp) {
      let userEntries = Object.entries(jsonData.users);
      if (!importSettings.isDev && userEntries.length > 2000)
        return {
          error:
            "You can only import up to 2000 users, unless you're a developer of the bot! Remove any invalid IDs, or users with low XP.",
        };
    ...
    if (!details.length)
      return {
        error: `No JSON data found! Syntax is { users: {...} }, settings: {...} }`,
      };
