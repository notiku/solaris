...
if (!fs.existsSync(autoPath + "status.json"))
  fs.copyFileSync("./json/default_status.json", autoPath + "status.json");
if (!fs.existsSync(autoPath + "version.json"))
  fs.writeFileSync(
    autoPath + "version.json",
    JSON.stringify({ version: "1.0.0", updated: Date.now() }, null, 2),
  );
...
const client = new Discord.Client({
  ...
  intents: [
    ...
    "DirectMessages",
    "GuildVoiceStates",
  ].map((i) => Discord.GatewayIntentBits[i]),
      ...
      ? [
          {
            ...
            type: Discord.ActivityType[status.type],
            url: status.url,
          },
        ]
      : [],
...
client.on("ready", () => {
  ...
  client.application.commands
    .fetch() // cache slash commands
    .then((cmds) => {
      if (cmds.size < 1) {
        // no commands!! deploy to test server
        console.info(
          "!!! No global commands found, deploying dev commands to test server (Use /deploy global=true to deploy global commands)",
        );
  ...
  // run the web server
  if (client.shard.id == 0 && config.enableWebServer)
    require("./web_app.js")(client);
});
...
client.on("interactionCreate", async (int) => {
  ...
  if (int.isStringSelectMenu()) {
    if (int.customId.startsWith("configmenu_")) {
      ...
      client.commands
        .get(configCmd)
        .run(client, int, new Tools(client, int), configData);
    }
    return;
  }
  ...
  // also for setting changes
    ...
    if (int.customId.startsWith("configmodal")) {
      ...
      client.commands
        ...
        .run(client, int, new Tools(client, int), modalData[1]);
    }
    return;
  }

  // general commands and buttons
  let foundCommand = client.commands.get(
    int.isButton() ? `button:${int.customId.split("~")[0]}` : int.commandName,
  );
