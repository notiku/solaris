...
if (!token)
  return console.log("No Discord token provided! Put one in your .env file");

const Shard = new Discord.ShardingManager("./index.js", { token });
const guildsPerShard = 2000;

Discord.fetchRecommendedShardCount(token, { guildsPerShard })
  .then((shards) => {
    let shardCount = Math.floor(shards);
    console.info(
      shardCount == 1 ? "Starting up..." : `Preparing ${shardCount} shards...`,
    );
